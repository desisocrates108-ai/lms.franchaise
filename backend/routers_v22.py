...
def _render_pdf_response(title: str, columns: list, rows: list, summary: dict, filename: str):
    ...
    elements = [
        Paragraph("<b>Servall Nexus</b>", h1),
...
def _render_invoice_pdf(inv: dict, vendor: dict) -> bytes:
    ...
    # Footer
    elements.append(Paragraph("<font color='#6b7280' size=8>Computer-generated invoice. Verified by Servall Nexus OCR engine.</font>", small))
...
def _render_dc_pdf(dc: dict, franchise: dict) -> bytes:
    ...
    elements = []
    header = Table([[
        Paragraph("<b>SERVALL NEXUS</b><br/><font size=8 color='#6b7280'>Hub & Spoke Distribution</font>", h1),
        Paragraph(f"<para align='right'><b>DELIVERY CHALLAN</b><br/>"
                  f"<font size=9 color='#6b7280'>{dc.get('dc_number', '')}</font></para>", sub),
    ]], colWidths=[270, 245])
    header.setStyle(TableStyle([
        ("BOTTOMPADDING", (0, 0), (-1, -1), 8),
        ("LINEBELOW", (0, 0), (-1, 0), 2, colors.HexColor("#111827")),
    ...
    # Sign-off
    elements.append(Paragraph("<para align='right'>Receiver's Signature ____________________________</para>", small))
    elements.append(Spacer(1, 6))
    elements.append(Paragraph("<font color='#6b7280' size=8>Generated by Servall Nexus. Scan QR to verify authenticity.</font>", small))
